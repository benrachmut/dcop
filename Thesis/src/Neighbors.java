...
public class Neighbors {
	...
	public void createFluds(double p3, int delayUB, Double p4) {
		...
		/*
		 * if (rnd<p3) { //this.isDelay12 = true; this.delayMap12 =
		 * this.setDelayMap(p3,delayUB,p4); } else { //this.isDelay12=false;
		 * this.delayMap12 = new HashMap<Integer, Integer>(); } rnd =
		 * Main.rProblem.nextDouble(); if (rnd<p3) { //this.isDelay21 = true;
		 * this.delayMap21 = this.setDelayMap(p3, delayUB,p4); } else {
		 * //this.isDelay21=false; this.delayMap21 = new HashMap<Integer, Integer>(); }
		 */
	...
	private Map<Integer, Integer> setDelayMap(double p3, int delayUB, Double p4) {
		...
		for (int i = 0; i < itirations; i++) {
			double rnd = Main.rDelay.nextDouble();
			int rndDelay;
			if (rnd < p3) {
				rndDelay = Main.getRandomInt(Main.rDelay, 1, delayUB);
				rnd = Main.rDelay.nextDouble();
				if (rnd < p4) {
	...
	public int getDelay12Synch(int currentIteration) {
	...
	public int getDelay12Unsynch() {
		counter12 = counter12+1;
		return delayMap12.get(counter12--);		
	}


	public int getDelay21Synch(int currentIteration) {
