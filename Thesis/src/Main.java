...
//----------------------------------------
	...
	static int iterations=50; // number of iterations
	...
	static int delayUpperBound=10;

	//static int itirationGap; // the gap of message delay
	static int costMax=10; // the max value of cost
		...
		for (Double p1 : p1s) {
			for (Double p2 : p2s) {
				for (int i = 0; i < meanReps; i++) {
					...
					for (Double p3 : p3s) {
						agentZero.changeCommunicationProtocol(p3,delayUpperBound);
...
/*
	...
	*/
	...
	private static Dcop createDcop(Double p1, Double p2) {
		agents = initAgentsFieldArray();
		Dcop dcop = new Dcop(agents, D, p1, p2,  delayUpperBound,  iterations);
	...
	private static void restartBetweenAlgo(Solution sol, double p3) {
